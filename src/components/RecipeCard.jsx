...
/* variable storing full recipe to condtionally render*/
  const renderFullRecipe = fullRecipe ? (
    <div id="fullRecipe" className="shadow" key={fullRecipe.id}>
      <img
        src={fullRecipe.image || placeholderImage}
        alt={fullRecipe.title || placeholderAlt}
      />
      <div id="textContent">
        <h3>{fullRecipe.title}</h3>
        <p>
          Ready in : {fullRecipe.readyInMinutes} Cooking time:
          {fullRecipe.cookingMinutes}
        </p>
  ...
  return secondRender ? renderFullRecipe : reccomendedCards;
}
